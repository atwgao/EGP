...
simul.r <- function(r,n,m,babu,sigma=NA,xi=NA,empiric=FALSE,seed=1){
  ...
  x <- x[x>0]
  bb<-m
  return(list(EGP1=egpd_s(m=m,x,model ="Pa",omega=0,badu=babu)$gamma,EGP2=egpd_s(m=m,x,model ="Pa",omega=0.1,badu=babu)$gamma,
              H=ReIns:::Hill(x)$gamma[bb:(n-1)],EPD=RevEPD(x)[bb:(n-1)]))
}
...
#par(mar=c(5,5,5,2))
plot(K,EGP1,type="l",xlab="K",ylab=bquote(EVI),ylim=c(-1,1),lwd=2.5,lty=1)
...
#par(mar=c(5,5,5,2))
plot(K,rmse_EGP1,type="l",xlab="K",ylab=bquote(RMSE),ylim=c(-0.3,1),lwd=2.5,lty=1)
