...
xi<-0.5;x<-rburr(n,1/xi,-0.5)#model4(n,-0.25)#rbeta(n,2,2)#rlnorm(n,0,1)#rgamma(n,1,1) #exp(rgamma(n,4,2))#rfrechet(n,1/xi)#
ms<-38
t1<-system.time({
  out<-function(i,o){
  ...
  estimators[,ms+2]<-egpd_s(m=1,x,model ="GP",omega=0,badu=T,mfun=ceiling(K^0.25))$gamma
  estimators[,ms+3]<-egpd_s(m=1,x,model ="GP",omega=0,badu=T,mfun=ceiling(K^0.5))$gamma
  estimators[,ms+4]<-egpd_s(m=1,x,model ="GP",omega=0,badu=T,mfun=ceiling(K^0.75))$gamma
  estimators[,ms+5]<-egpd_s(m=1,x,model ="GP",omega=0,badu=T,mfun=ceiling(K^0.9))$gamma
  ...
  i<-23
   EPD(x,plot=T,col=1,lwd=5)
   GPDmle(x,add=T,ylim=c(-1,2),main="Estimatates of the EVI")
   lines(estimators[,12],col=2,lwd=3)
   lines(i:(n-1),result[[i]],col=i+1,lwd=2)
   abline(h=xi)
   lines(estimators[,ms+5],lwd=4,col="red4")
  #abline(h=xi)
  # 
